...
class PPO_Switch:
    ...
    def DRL_prediction(self, model, environment, deterministic=True):
        ...
        model_selection_counts = [0 for _ in range(len(model))]
        ...
        for i in range(len(environment[0].df.index.unique())):
            ...
            if i >= 10:
                ...
                if i % switchWindow == 0:
                    ...
                    model_selection_counts[lastSwitchModelIndex] += 1
        ...
        model_names = ["Real", "Max", "Min", "Mean", "EMA"]
        stats = ", ".join([f"{name}: {count}" for name, count in zip(model_names, model_selection_counts)])
        print(f"\n모델별 선택 횟수: {stats}\t sum: {sum(model_selection_counts)}")
    ...
    def sparse_action(self, env, action, stocks_num):
        ...
        newAction = np.array(heldQuantity)/-self.hmax
